...
TESTING = False
tenant = Tenant.create_by_name('peaceful-fire')
...
def delete_all_items(item_name):
  endpoint_url = tenant.api_base + f'/{item_name}'
...
def reset_instance():
  ...
  delete_all_items('admin/users')

  delete_all_items('global_resources')
  delete_all_items('user_credentials')
  batch_delete_stories()
  
  delete_all_items('folders')
  delete_all_items('teams')
